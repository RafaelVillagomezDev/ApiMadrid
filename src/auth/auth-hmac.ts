...
export const verifyHmacAuthenticity = (req: Request,
    ...
    if (!signature || !timestampStr || !nonce) {
        return res.status(401).json({ message: 'Faltan parámetros de autenticidad.', code: 401 });
    }
    ...
    if (Math.abs(currentTime - timestamp) > TIMESTAMP_TOLERANCE_MS) {
        return res.status(401).json({ message: 'La solicitud ha caducado. Tenga sincronizado el reloj.', code: 401 });
    }
    ...
    if (!crypto.timingSafeEqual(Buffer.from(signature), Buffer.from(expectedSignature))) {
        return res.status(401).json({ message: 'Firma de solicitud inválida. Datos alterados.', code: 401 });
    }
