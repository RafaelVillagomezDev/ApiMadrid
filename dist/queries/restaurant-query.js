...
exports.countTotalRestaurants = exports.removeRestaurantsData = exports.formatRestaurantData = exports.getRestaurantData = exports.isRestaurant = exports.existRestaurant = exports.createRestaurant = void 0;
...
const countTotalRestaurants = ({ id, name, address, type_food }) => {
    const conditions = [];
    const filterValues = [];
    if (id) {
        conditions.push(`id = ?`);
...
exports.countTotalRestaurants = countTotalRestaurants;
const getRestaurantData = ({ id, name, address, type_food, limit, offset, }) => {
    const conditions = [];
    const filterValues = [];
    if (id) {
        conditions.push(`id = ?`);
...
const formatRestaurantData = (rows) => {
    ...
    for (const row of rows) {
        ...
        if (row.image_id &&
            !restaurant.images.find((img) => img.id === row.image_id)) {
            restaurant.images.push({ id: row.image_id, url: row.image_url });
        }
        // Buscar si el menú ya está agregado
        let menu = restaurant.menus.find((m) => m.id === row.menu_id);
        if (!menu && row.menu_id) {
            menu = {
                id: row.menu_id,
                name: row.menu_name,
        ...
        if (menu &&
            row.dish_id &&
            !menu.dishes.find((d) => d.id === row.dish_id)) {
            menu.dishes.push({
                id: row.dish_id,
                name: row.dish_name,
...
exports.formatRestaurantData = formatRestaurantData;
const removeRestaurantsData = () => {
    const query = `DELETE FROM restaurant WHERE id = ?;`;
    return query;
};
exports.removeRestaurantsData = removeRestaurantsData;
